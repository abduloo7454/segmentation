...
Note: This model is trained on the COCO/VOC dataset (general objects), so segmentation results on medical images may not be medically accurate but demonstrate model operation.
""")
...
if uploaded_file is not None:
    ...
    base_colors = plt.get_cmap("tab20").colors  # 20 base colors
    ...
    for label_index in range(num_classes):
        color = tuple(int(x) for x in colormap[label_index % len(colormap)])
    ...
    # Category legend with robust color formatting
    legend_items = []
    categories = weights.meta['categories']
    for i in range(num_classes):
        color = tuple(int(x) for x in colormap[i % len(colormap)])
        color_hex = '#%02x%02x%02x' % color
        legend_items.append(
            f'<span style="background-color:{color_hex};padding:2px 8px;border-radius:3px;display:inline-block;margin-right:8px;">{categories[i]}</span>'
        )
    st.markdown(
        "**Category Color Legend:**<br>" + " ".join(legend_items),
